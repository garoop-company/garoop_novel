...
function BabyCard({ baby, isOwn = false, onChat }: { baby: PublicBaby; isOwn?: boolean; onChat: (baby: PublicBaby) => void }) {
  ...
  // パネルは右端のスクロール枠内なので、詳細は viewport 基準(fixed)で左側に開く
  // ＝ overflow に見切られず、画面外にもはみ出さない
  const toggle = () => {
    setOpen(v => {
      const next = !v
      if (next && btnRef.current) {
        const r = btnRef.current.getBoundingClientRect()
        setCoords({ top: r.top + r.height / 2, right: window.innerWidth - r.left + 12 })
      }
      return next
    })
  }

  // fixed のポップオーバーがズレないよう、スクロール/リサイズで閉じる
  useEffect(() => {
    if (!open) return
  ...
  return (
    <div className="relative">
      <button
        ref={btnRef}
        onClick={toggle}
        className={`relative flex flex-col items-center gap-1.5 group ${isOwn ? "scale-110" : ""}`}
      >
        ...
        <span className="text-[10px] text-amber-300/80 max-w-[60px] truncate">{baby.name}</span>
      </button>

      {open && coords && (
        <div
          style={{ position: "fixed", top: coords.top, right: coords.right, transform: "translateY(-50%)" }}
          className="z-50 w-52 bg-slate-900 border border-amber-400/30 rounded-2xl p-3.5 shadow-2xl text-xs"
        >
...
export function BabyCompanion() {
  ...
  return (
    <>
      ...
      <div className="hidden wide:flex fixed right-3 top-1/2 -translate-y-1/2 z-40 flex-col items-center max-h-[86vh]">
        <div className="flex flex-col items-center gap-4 overflow-y-auto max-h-[74vh] scrollbar-none px-2 py-5 rounded-[28px] bg-slate-950/35 border border-amber-200/10 backdrop-blur-sm shadow-xl">
          {allBabies.map(b => (
            <BabyCard key={b.id} baby={b} isOwn={myBabyIds.has(b.id)} onChat={setChatBaby} />
          ))}
          {allBabies.length === 0 && !loading && (
            <div className="text-[10px] text-slate-500 text-center leading-relaxed px-1 py-2">赤ちゃん<br />まだ<br />いないよ</div>
          )}
        </div>
      </div>
